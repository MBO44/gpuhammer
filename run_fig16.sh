...
for i in 0 5; do            # for the two bitflips used in the paper
    ...
    for j in {0..15}; do
        ...
        for k in {0..15}; do
            ...
            # File paths
            mkdir -p "$HAMMER_ROOT/results/fig16/raw_data"
            rowset_file="$HAMMER_ROOT/results/row_sets/ROW_SET_${bank_id}.txt"
            log_file="$HAMMER_ROOT/results/fig16/raw_data/${num_agg}agg_b${bank_id}_flip${flip_row}_${pats[$j]}${pats[$k]}.log"
            bitflip_file="$HAMMER_ROOT/results/fig16/raw_data/${num_agg}agg_b${bank_id}_count_flip${flip_row}_${pats[$j]}${pats[$k]}.txt"
