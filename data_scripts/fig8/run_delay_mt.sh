...
result_file="$HAMMER_ROOT/results/fig8/mt_delays.txt"
...
# Running the test
for i in {2..32}; do
    $HAMMER_ROOT/src/out/build/mt_sync_delay $rowset_file $(($i - 1)) $addr_step $iterations $rowid $mem_size $time_file $num_warp $num_thread $round 0 1 $num_rows >> $log_file
    sleep 3
