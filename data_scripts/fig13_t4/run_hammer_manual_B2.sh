...
delay=58
aggressor_row=13059
victim_byte_offset=10268835328
aggressor_row_offset=10270674176
...
store_dir=$HAMMER_ROOT/results/fig13_t4/B2
mkdir -p $store_dir
for model in alexnet vgg resnet dense inception; do
    ...
    shuf --random-source=<(yes 42) -i 0-$shift_range -n 50 | while read num; do
        ...
        sleep 5
        python3 $HAMMER_ROOT/util/run_imagenet_models.py $model att B2 $HAMMER_ROOT/results/fig13_t4/ $HAMMER_ROOT/src/out/build/liballoc.so $store_dir/${model}.txt
        sleep 5
        > memory_control.txt
        sleep 5
        echo "Hammering done."
