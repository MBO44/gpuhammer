...
static uint64_t next_conf_step(rbce::N_Conflict &nc_test,
                               uint64_t step /* step to source address */,
                               uint64_t base_delay, uint64_t threshold);

static bool in_bank_set(rbce::N_Conflict &nc_test, Bank_Map &bank_map,
                        uint64_t step, uint64_t base_delay, uint64_t threshold);

int main(int argc, char *argv[])
{
  /* Argument EXP_RANGE, EXP_IT, STEP */
  rbce::N_Conflict nc_test(2, std::stoull(argv[1]), std::stoull(argv[1]),
  ...
  nc_test.loop_range(
      [&nc_test, &bank_map, threshold](uint64_t step)
      {
        ...
        if (!in_bank_set(nc_test, bank_map, step, base_delay, threshold))
        {
          ...
          bank_map[step] = {
              base_delay, next_conf_step(nc_test, step, base_delay, threshold)};
          // std::cout << bank_map.size() <<'\n';
        }
      },
 ...
 * @return uint64_t
 */
uint64_t next_conf_step(rbce::N_Conflict &nc_test, uint64_t this_step,
                        uint64_t base_delay, uint64_t threshold)
{
 ...
 */
bool in_bank_set(rbce::N_Conflict &nc_test, Bank_Map &bank_map,
